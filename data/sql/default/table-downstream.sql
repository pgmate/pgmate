WITH dependencies AS (
  ...
  WHERE ref_namespace.nspname = 'public'
    AND ref_class.relname = 'sales_invoices'
    AND dep.deptype != 'i'
    AND obj_class.oid <> ref_class.oid    -- Exclude self-dependency
  ...
  WHERE con.contype = 'f'
    AND pk_table_ns.nspname = 'public'
    AND pk_table.relname = 'sales_invoices'
  GROUP BY con.conname, fk_table_ns.nspname, fk_table.relname, pk_table_ns.nspname, pk_table.relname

  UNION ALL

  -- Dependencies through views/materialized views
  SELECT
    'v' AS deptype,
  ...
  WHERE tbl_namespace.nspname = 'public'
    AND tbl_class.relname = 'sales_invoices'
    AND vw_class.oid <> tbl_class.oid    -- Exclude self-dependency
)
SELECT
